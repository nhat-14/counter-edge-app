...
oras push harbor.machine:8443/library/edge-nhat-app-package:latest \
...
cd /home/azureuser/workspace/sandbox/scripts

echo "Uploading application package.."
sudo -E bash wfm-cli.sh upload-app-non-interactive edge-nhat-app-package
if [ $? -ne 0 ]; then
	echo "Failed to upload application package for edge-nhat-app-package."
	exit 1
fi
